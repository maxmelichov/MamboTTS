# Adding open-source models to MamboTTS

MamboTTS downloads model assets at runtime, but **inference engines are compiled into the app**. There is no plugin/DLL loader for third-party TTS engines. New open-source models ship only after they are registered in the repo and merged through a pull request.
...
```text
crates/mambotts-registry   ← catalog: names, sizes, download URLs, capabilities
        ↓
desktop downloader (Tauri)   ← fetches files into the app data `models/` folder
        ↓
mambotts-server sidecar    ← loads files through a compiled Runtime adapter
        ↓
...
- Updating a download URL or file list means changing `mambotts-registry` and usually cutting a new app release.
...
1. Fork [maxmelichov/MamboTTS](https://github.com/maxmelichov/MamboTTS) and create a branch (`feat/add-<runtime-id>`).
2. Host or point to open-source model files with stable URLs.
3. Register the model in `crates/mambotts-registry`.
4. Wire load + inference in `mambotts-server` (new runtime) or verify paths still load (asset-only change).
...
2. **Edit** [`crates/mambotts-registry/src/lib.rs`](../crates/mambotts-registry/src/lib.rs):
...
3. **Keep desktop paths aligned** if the desktop still hardcodes Blue filenames or helper URLs (see [`mambotts-desktop/src-tauri/src/model.rs`](../mambotts-desktop/src-tauri/src/model.rs)). Prefer reading from the registry where possible; if a temporary constant remains, update it in the same PR.
4. **Server loading** — if filenames change, update Blue load validation and any path defaults in:
   - [`mambotts-server/src/server/handlers/load.rs`](../mambotts-server/src/server/handlers/load.rs)
   - [`mambotts-server/src/runtime/blue.rs`](../mambotts-server/src/runtime/blue.rs)
   - desktop runner DTOs / load requests under `mambotts-desktop/src-tauri/src/runner/`
...
In [`crates/mambotts-registry/src/lib.rs`](../crates/mambotts-registry/src/lib.rs):
...
Implement [`Runtime`](../mambotts-server/src/runtime/mod.rs) in a new module under `mambotts-server/src/runtime/`, then export it from `runtime/mod.rs`.
...
Add a `RuntimeParams` variant for the paths/config your loader needs, and branch on it in [`Server::load_model`](../mambotts-server/src/server/state.rs).

Update [`POST /v1/models/load`](../mambotts-server/src/server/handlers/load.rs) so the new `runtime` id is accepted and validated (Blue-only checks must become a match on runtime id).

If you introduce a Rust crate for the engine (like `crates/blue-rs`), keep ONNX / native deps explicit and document packaging needs for `scripts/pre_build.py`. Prefer engines that link a prebuilt runtime over ones that compile a native inference stack from source, since a from-source build has to work on every platform the app ships to.

Remember that the text front end is part of the model. Blue reads IPA rather than Hebrew script, which is why `renikud-plus.onnx` is in its `required_files` and a missing G2P model fails at load time instead of at the first Hebrew request. If your checkpoint has the same shape, declare its front end the same way.

### 3. Desktop download and load wiring

Minimum desktop work:

1. [`mambotts-desktop/src-tauri/src/model.rs`](../mambotts-desktop/src-tauri/src/model.rs) — implement download for the new runtime id (today Blue is special-cased; generalize or add a clear branch). Respect `InstallKind::Files` vs archive extraction.
2. Runner client / DTOs — send the correct load payload (`runtime`, `model_path`, and any engine-specific fields).
3. TypeScript types in [`mambotts-desktop/src/lib/types.ts`](../mambotts-desktop/src/lib/types.ts) if new fields appear.
4. Onboard / settings UI — the catalog cards come from registry sources; ensure selecting the new runtime downloads, persists `mambotts.runtime`, and reloads the sidecar.
...
## Implementation
- [ ] Registry manifest updated (`crates/mambotts-registry`)
...
2. Point load paths at local ONNX/GGUF files via the load API fields or environment variables currently recognized by the Blue loader (`MAMBOTTS_BLUE_MODEL_DIR`, `MAMBOTTS_RENIKUD_PATH`, etc.).
...
| --- | --- |
| Catalog / URLs / capabilities | `crates/mambotts-registry/src/lib.rs` |
| HTTP sources API | `mambotts-server/src/server/sources.rs` |
| Load API | `mambotts-server/src/server/handlers/load.rs` |
| Runtime trait + params | `mambotts-server/src/runtime/mod.rs` |
| Blue reference adapter | `mambotts-server/src/runtime/blue.rs` |
| Downloads | `mambotts-desktop/src-tauri/src/model.rs` |
